...
void *StoreLoader() {
  ...
  while (!release_condition) {
    ...
    printf("Storehouse %d is loaded with %d goods. Current amount: %d\n",
      chosen_store + 1, goods_amount, storehouse[chosen_store]);
...
//if buyer wants more than he needs and storehouse have more than he needs
...
void *BuyGoods(void *chosen_buyer) {
  ...
  while (buyer_demands[buyer_num] != 0) {
    ...
    printf("Buyer %d bought %d goods from store %d. Demands: %d\n",
      buyer_num + 1, goods_bought, chosen_store, buyer_demands[buyer_num]);
    printf("Store %d goods left: %d\n", chosen_store + 1, storehouse[chosen_store]);
    pthread_mutex_unlock(&store_lock[chosen_store]);
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < 3; i++) {
    pthread_create(&buyer[i], &attr, BuyGoods, (void *)i);
  }
